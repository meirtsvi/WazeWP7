...
void roadmap_dialog_new_choice (const char *frame,
   ...
   int i;
#ifdef RIMAPI
   NOPH_ChoiceGroup_t item = NOPH_ChoiceGroup_new (name, NOPH_Choice_EXCLUSIVE);
#else
...
void  roadmap_dialog_set_data (const char *frame, const char *name,
   ...
   if (data_is_string) {
      if (strlen(this_item->value)) free(this_item->value);
      if (!strlen((char *)data)) this_item->value = "";
      else this_item->value = strdup((char *)data);
   } else {
