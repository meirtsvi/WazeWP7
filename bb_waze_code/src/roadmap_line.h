...
extern RoadMapLine *RoadMapLineActiveLine;
...
#endif
   roadmap_point_position (RoadMapLineActiveLine[line].from, position);
}
...
#endif
   roadmap_point_position (RoadMapLineActiveLine[line].to, position);
}

#ifdef J2ME
INLINE_DEC void roadmap_line_fast_points (int line, RoadMapPosition *from, RoadMapPosition *to) {

   RoadMapLine *linep = &RoadMapLineActiveLine[line];

   int t = *(int *)(&linep->from);
   int from_p = (t >> 16) & POINT_REAL_MASK;
   int to_p = t & POINT_REAL_MASK;
   
   roadmap_point_position_safe (from_p, from);
   roadmap_point_position_safe (to_p, to);
}


INLINE_DEC void roadmap_line_fast (int line, int *first_shape, int *last_shape, RoadMapPosition *from, RoadMapPosition *to) {

   RoadMapLine *linep = &RoadMapLineActiveLine[line];

   //FIXME J2ME is limited to 32k shapes in a square while buildmap supports 64k
   *first_shape = *(int *)(&linep->first_shape);
   if (*first_shape < 0) {
      *first_shape = -1;
   } else {
      int count;

      *first_shape = (*first_shape >> 16);
      /* the first shape is actually the count */
      count = roadmap_shape_get_count (*first_shape);

      *last_shape = *first_shape + count;
      (*first_shape)++;
   }
...
INLINE_DEC int roadmap_line_shapes (int line, int *first_shape, int *last_shape) {
   ...
   shape = RoadMapLineActiveLine[line].first_shape;
   if (shape == ROADMAP_LINE_NO_SHAPES) {
      *first_shape = -1;
      return -1;
   }
