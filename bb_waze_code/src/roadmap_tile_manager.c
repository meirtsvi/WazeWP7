...
static int  http_cb_size (void *context, size_t size) {
   ...
   if (size> 100000) {
	   roadmap_log(ROADMAP_ERROR,"Size for %s = %d is too large",conn->url, size);
   }
...
static void http_cb_done (void *context,char *last_modified) {
   ...
   *tile_status = ((*tile_status) |
   ...
   if (roadmap_screen_in_view(tile_index)) {
...
static void get_url (ConnectionContext *context) {
    ...
    snprintf (context->url,
				 sizeof (context->url),
				 "%s_j2me/%05d_%02x/%05d_%04x/%05d_%06x/%05d_%08x%s",
				 get_url_prefix (),
				 fips, tile_id >> 24,
				 fips, tile_id >> 16,
				 fips, tile_id >> 8,
...
static void load_next_tile (void) {
   ...
   roadmap_log(ROADMAP_DEBUG, "load_next_tile - status:%d", Status);
	...
	//printf ("Requesting %d %s\n", Connections[conn].tile_index, Connections[conn].url);
