...
static void roadmap_download_start (void) {
   ...
   if ((resData.type == RES_DOWNLOAD_IMAGE) || (resData.type == RES_DOWNLOAD_COUNTRY_SPECIFIC_IMAGES)){
	  ...
	  /*
#ifdef RIMAPI
      roadmap_file_remove(NULL, res_path);
      strcpy(strrchr(res_path,'.'),".bin");
#endif// RIMAPI
	  */
   }
   ...
   if (roadmap_file_exists (NULL, res_path) && !resData.override) {
      ...
      context->data = NULL;
      roadmap_log(ROADMAP_DEBUG,"Downloading resource.  type=%d, name=%s, lang=%s url=%s" ,resData.type, resData.name, resData.lang,url);
      // Start the process
...
static void download_done_callback (void *context_cb, char *last_modified) {
   ...
   roadmap_log(ROADMAP_ERROR,"Download finished downloading (%s) Writing %d bytes, last modified = %s" , path, context->data_size, last_modified );
