...
static RoadMapImage create_sides_image (int style, int header, int pointer_type, RoadMapGuiPoint *bottom, RoadMapGuiPoint *top, int num_items) {
   ...
   if(s_images[border_white_left+style].height * num_items > roadmap_canvas_height())
	   image_height =  roadmap_canvas_height();
   else
	   image_height = s_images[border_white_left+style].height * num_items;

   image = roadmap_canvas_new_image (image_width,image_height);
...
//   for (i = 0; i<num_items; i++){
      ...
      point.y = 0;//i* s_images[border_white_right+style].height;
//      if (point.y > roadmap_canvas_height()){
//    	  break; // don't create side images that have a bigger height than the screen
//      }
	  roadmap_canvas_copy_image (image, &point, rect, s_images[border_white_right+style].image, CANVAS_COPY_NORMAL);

      point.x = 0;
      point.y = 0;//i* s_images[border_white_left+style].height;
      roadmap_canvas_copy_image (image, &point, rect, s_images[border_white_left+style].image, CANVAS_COPY_NORMAL);
//   }
...
//      for (i = 0; i<num_items; i++){
         ...
         point.y = 0;
         roadmap_canvas_copy_image (image, &point, rect, s_images[border_white_top+style].image, CANVAS_COPY_NORMAL);
//      }
...
//      for (i = 0; i<num_items; i++){
         ...
         point.y = 0;
         roadmap_canvas_copy_image (image, &point, rect, s_images[header+1].image, CANVAS_COPY_NORMAL);
//      }
...
//      for (i = 0; i<num_items; i++){
         ...
         point.y = 0;
         roadmap_canvas_copy_image (image, &point, rect, s_images[border_white_bottom+style].image, CANVAS_COPY_NORMAL);
//      }
...
int roadmap_display_border(int style, int header, int pointer_type, RoadMapGuiPoint *bottom, RoadMapGuiPoint *top, const char* background, RoadMapPosition *position){
	...
	int i, num_items, sides_image_height, sides_height, end_sides_y;
	int count, start_pos_x, top_height;
	RoadMapGuiPoint sign_bottom, sign_top,in_screen_sides_starting_point;
	RoadMapImage image;
...
#ifdef J2ME	
	sides_height = ( sign_height - start_sides_point.y +sign_top.y - s_images[border_white_bottom+style].height );
	num_items =  sides_height/ s_images[border_white_right+style].height;

    if (num_items > 0) {
	   image  = get_image(border_sides, style, header, pointer_type, bottom, top, num_items+1);
	   sides_image_height = roadmap_canvas_image_height(image);
	   point.x = left_point.x;
	   point.y = start_sides_point.y ;
	   end_sides_y = point.y + sides_height;
	   in_screen_sides_starting_point.x = point.x;
	   /*
	    * As an optimization ( crucial in BB ), we will never create sides images which are larger
	    * than the screen size. The following logic takes care that we still draw the sides
	    * image in the correct place. - D.F.
	    */

	   if (point.y<0){
		   if(end_sides_y < 0 )
			   in_screen_sides_starting_point.y  = -1; // ending point is above screen, don't draw
		   else if(end_sides_y > screen_height)
			   in_screen_sides_starting_point.y  = 0;
		   else // end_y is in screen
			   in_screen_sides_starting_point.y = end_sides_y - sides_image_height;
	   }else if(point.y>screen_height){ // starting point is below screen , don't draw
		   in_screen_sides_starting_point.y = -1;
	   }else{ // point.y is in screen
		   in_screen_sides_starting_point.y = point.y;
	   }

	   if(in_screen_sides_starting_point.y!=-1)
	   {
		   roadmap_canvas_draw_image (image, &in_screen_sides_starting_point, 0, IMAGE_NORMAL);
	   }

	   point.y = end_sides_y -1;
    }
#else
	...
	RoadMapImage bottom_image;
	left_point.y = point.y +  s_images[border_white_left+style].height ;
	bottom_image =  get_image(border_bottom, style, header, pointer_type, bottom, top, 0);
	roadmap_canvas_draw_image (bottom_image, &left_point, 0, IMAGE_NORMAL);
#ifndef __ROADMAP_BORDER_CACHE__
	// Release after drawing
	roadmap_canvas_free_image( bottom_image );
#endif
		...
		if (pointer_type == POINTER_POSITION){
			...
			if (visible){
				...
				if (points[0].y > (point.y +10) ){
	    			...
	    			pointer_pen = roadmap_canvas_create_pen ("fill_pop_up_pen", FALSE);
	
	    			roadmap_canvas_set_foreground("#000000");
	    		   roadmap_canvas_set_opacity(opacity);
	    		   count = 3;
	...
	roadmap_canvas_create_pen ("fill_pop_up_pen", FALSE);
    roadmap_canvas_set_foreground(background);

   if (style == STYLE_BLACK)
      roadmap_canvas_set_opacity(opacity);
#ifdef RIMAPI      
   RoadMapGuiRect *rect = (RoadMapGuiRect *)malloc(sizeof(RoadMapGuiRect));
   rect->minx = fill_points[2].x; 
   rect->miny = fill_points[2].y;
   rect->maxx = fill_points[0].x-1;
   rect->maxy = fill_points[0].y-1;
   // this will color the rect with the right colors, since erase fill in with 
   // the selected color and opacity
