...
int roadmap_label_add (const PluginLine *line, const LabelInfo *label) {
   ...
   if (label->length_sq < RoadMapLabelMinFeatSizeSq) {
      return -1;
   }
   ...
   cPtr->line = *line;
   cPtr->featuresize_sq = label->length_sq;
   cPtr->angle = normalize_angle(roadmap_math_azymuth(&label->p1, &label->p2));

   cPtr->center_point = label->center;
   ...
   if (cPtr->text != NULL && cPtr->text[0] != '\0')
	cPtr->textHash = roadmap_hash_string(cPtr->text);
   else
	   cPtr->textHash = 0;
