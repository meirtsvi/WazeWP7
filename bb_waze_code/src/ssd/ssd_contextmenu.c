...
#ifdef RIMAPI
void static ssd_contextmenu_set_native_menu(ssd_contextmenu_ptr menu);
#endif
void cm_context_reset( cm_context_ptr this)
{
                           ...
                           int               selected_item)
{
   ...
   activator_offset  = (int)(((double)s_row_height * (selected_item + 1))+0.5);
...
{
   const char *         row_val    = ssd_widget_get_value( this, "label");
   ...
   for( i=0; i<menu->item_count; i++)
   {
      if( !strcmp( menu->item[i].label, row_val))
      {
         menu->item_selected = i;
         break;
      }
   ...
   for( i=0; i<menu->item_count; i++)
   {
      ...
      else
      {
        ...
        if( item->icon)
        {
         ...
         ssd_widget_set_color( row, "#000000", NULL);
      }
   ...
   for( i=0; i<menu->item_count; i++)
   {
      ...
      if( CONTEXT_MENU_FLAG_POPUP & item->flags)
      {
         ...
         if( SSD_CONTEXTMENU_SIMPLE_LIST & flags)
            ...
            ssd_widget_set_color( popup_cnt, "#000000", NULL);
                              ...
                              unsigned short       flags)
{
   ...
   if( !s_row_height)
   {
      ...
      s_row_height = 5 + (text_ascent + text_descent);
      if (s_row_height < (ROW_ICON_MAX_HEIGHT + 5)) s_row_height = ROW_ICON_MAX_HEIGHT + 5;
   }
...
#ifdef TOUCH_SCREEN
         if( SSD_CONTEXTMENU_SIMPLE_LIST & flags)
            ssd_widget_set_size  ( item->row, text_width, s_row_height);
         else
         {
   ...
   if( SSD_CONTEXTMENU_SIMPLE_LIST & flags)
   {
      ...
      container_height  = 4 + (used_rows_count * s_row_height);
   }
   else
   {
#ifdef TOUCH_SCREEN
      ...
      if (menu_cnt->flags & SSD_POINTER_MENU)
         container_height  =  30 + (used_rows_count * s_row_height);
      else
         container_height  =  10 + (used_rows_count * s_row_height);
#endif
   }
                     ...
                     unsigned short       flags)
{
   ...
   if( SSD_CONTEXTMENU_SIMPLE_LIST & flags)
      ...
      ssd_widget_set_color( menu_cnt, "#000000", NULL);
...
BOOL ssd_context_menu_show(int                  x,
                           int                  y,
                           ssd_contextmenu_ptr  menu,
...
#ifdef RIMAPI
   ssd_contextmenu_set_native_menu(menu);
   return FALSE;
#endif
...
// will be called once a menu item is clicked
void native_menu_item_callback(int ordinal){
	exit_context_menu(TRUE,s_ctx.menu->item+ordinal-1);
}


// will be called from on_options when menu button is clicked
void static ssd_contextmenu_set_native_menu(ssd_contextmenu_ptr menu){
	int i ;
	ssd_cm_item_ptr item = NULL;
	for(i=0; i<menu->item_count;i++){
		ssd_cm_item_ptr item = menu->item + i;
		if( !(CONTEXT_MENU_FLAG_HIDDEN & menu->item[i].flags))
			roadmap_main_context_menu_set(i+1, roadmap_lang_get(item->label), native_menu_item_callback);
	}
}

void on_options_wrapper(int callback_addr){
	SsdWidget container = ssd_dialog_get_current_container();
	if(container)
		(*(SsdSoftKeyCallback)callback_addr) (container, container->name, container->context);
	else
		(*(SsdSoftKeyCallback)callback_addr) (NULL, NULL, NULL);
}

void  ssd_contextmenu_menu_button_register(SsdSoftKeyCallback callback,const char * dialog_name){
	// if callback isn't known, send with value 0 - The callback might be found in the callback cache
	// in roadmap_main
	if(callback!=NULL)
		roadmap_main_menu_button_register(on_options_wrapper,dialog_name,(int)callback);
    else
    	roadmap_main_menu_button_register(on_options_wrapper,dialog_name,0);
}
#endif
