...
static void kill_messagebox_timer (int exit_code, void* context) {
...
static void update_button(void){
   ...
   ssd_widget_reset_cache (s_gMsgBoxDlg);
...
static SsdWidget create_messagebox ( const char* mb_name )
{
   ...
   char button_txt[20];
   dialog = ssd_dialog_new ( mb_name, "", kill_messagebox_timer,
				...
				int title_font_size, char* title_color, int text_font_size, char* text_color )
{
   ...
   if(strlen(text) > SSD_TEXT_MAXIMUM_TEXT_LENGTH)
   	ssd_text_set_text_size(widget_text, strlen(text)+1);
   ssd_text_set_text( widget_text, text );


   ssd_widget_reset_cache (dialog);
