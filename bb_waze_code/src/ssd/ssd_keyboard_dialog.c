...
#ifndef RIMAPI
#define SSD_KB_DLG_EDIT_FONT_SIZE           18
#else
#define SSD_KB_DLG_EDIT_FONT_SIZE           15
#endif
...
int on_options( SsdWidget widget, const char *new_value, void *context)
{
   ...
   s_context_menu_is_active =   ssd_context_menu_show(  menu_x,              // X
								   ...
								   int kb_dlg_flags )
{
   ...
   int entry_height = ssd_container_get_txtbox_height();
  ...
  // assert(!s_cbOnDone);
   ...
   if( !ssd_dialog_exists( s_dialog_name ) )
   {
      ...
      edit = ssd_text_new     ( s_editbox_name, "", SSD_KB_DLG_EDIT_FONT_SIZE, SSD_ALIGN_VCENTER|SSD_TEXT_INPUT );
   ...
   else
   {
	...
	if ( kb_dlg_flags & SSD_KB_DLG_INPUT_ENGLISH )
			ssd_text_set_input_type ( edit, inputtype_simple_text);
	else
			ssd_text_set_input_type ( edit, inputtype_free_text);		
