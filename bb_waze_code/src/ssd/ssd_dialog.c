...
SsdWidget ssd_dialog_get_current_container(void){
...
#endif// TOUCH_SCREEN||POINTER_SCREEN
...
static int draw_dialog (SsdDialog dialog) {
...
#ifdef RIMAPI
	if(RoadMapDialogCurrent)
		ssd_contextmenu_menu_button_register(NULL,RoadMapDialogCurrent->name);
	else
		ssd_contextmenu_menu_button_register(NULL,"");
#endif
...
static void left_softkey_callback(void){
   ...
   if (current == NULL)
     ...
     if (current->scroll_container && current->scroll_container->left_softkey_callback)
      ...
      (*current->container->left_softkey_callback)(current->container, "", current->container->context);
    else
         ssd_dialog_hide_all(dec_cancel);
...
static void right_softkey_callback(void){
   ...
   if (current->scroll_container && current->scroll_container->right_softkey_callback)
      ...
      (*current->container->right_softkey_callback)(current->container, "", current->container->context);
   else
         ssd_dialog_hide_current(dec_cancel);
}
...
#ifdef RIMAPI
   ssd_contextmenu_menu_button_register(NULL, name);
#endif
...
#ifdef RIMAPI
   roadmap_main_menu_button_unregister(dialog->name);
...
#ifdef RIMAPI
      if(dialog)
	     ssd_contextmenu_menu_button_register(NULL,dialog->name);
#endif
   }
