...
static void init_containers (void) {
   bg = roadmap_canvas_create_pen ("container_bg", FALSE);
   roadmap_canvas_set_foreground (default_bg);

   border = roadmap_canvas_create_pen ("container_border", FALSE);
...
static void draw_bg(RoadMapGuiRect *rect){
   ...
   if (roadmap_skin_state()){
      if (night_pen == NULL){
         roadmap_canvas_create_pen("container_bg_pen_night", FALSE);
      ...
      if (day_pen == NULL){
         roadmap_canvas_create_pen("container_bg_pen_day", FALSE);
...
int ssd_container_get_txtbox_height(){
	int height = 40;
...
static void add_title (SsdWidget w, int flags) {
  ...
  ssd_widget_set_color (title, "#ffffff", NULL);
