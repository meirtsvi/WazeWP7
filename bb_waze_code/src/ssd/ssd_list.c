...
static int label_callback (SsdWidget widget, const char *new_value) {
   ...
   const char * text = ssd_widget_get_value (widget, "label");
   ...
   return (*data->callback) (list, ssd_widget_get_value (widget, "label"),
             data->data ? (void *)data->data[data->selected_row ] : NULL);
}

#ifdef TOUCH_SCREEN
static int ssd_list_short_click (SsdWidget widget, const RoadMapGuiPoint *point) {
...
static int delete_callback (SsdWidget widget, const char *new_value) {
   ...
   return (*data->del_callback) (list, text, data->data ? (void *)data->data[absolute_index ] : NULL);
}
...
static BOOL move_focus( SsdWidget this, BOOL up /* or down */)
{
   ...
   if( up)
   {
      ...
      {
#ifdef RIMAPI
            // no wrap around scrolling in blackberry, do nothing. 
#elif defined(J2ME)       
			if(list_data->rows[(list_data->num_values - 1)]->position.y==-1)
   ...
   else
   {
      ...
      {
#ifndef RIMAPI      // no wrap around in blackberry       
