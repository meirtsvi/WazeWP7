...
static int edit_callback (SsdWidget widget, const char *new_value) {
...
#if (defined(__SYMBIAN32__) && !defined(TOUCH_SCREEN)) || defined (RIMAPI) || defined (IPHONE) || defined (ANDROID)
   {
	   ...
	   const char *box_title= ctx->editbox_title;
	   if (!box_title)
		   box_title= ctx->kb_title;
	   //printf("ShowEditbox flags 0x%x tit %s val %s",text->flags ,box_title,value);
