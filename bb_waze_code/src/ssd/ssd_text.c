...
#define MAX_NUMBER_CACHED_LINES 5
...
typedef struct tag_text_ctx {
	...
	// the next are added support the caching of line breaks - DF

	//array of pointers to the original text - line_starts[i] holds the beginning of the i'th line on screen.
	char* line_starts[MAX_NUMBER_CACHED_LINES];

	/*
	 * '\0' characters will be inserted between lines instead of the original values of ' ' or \n.
	 * this array will hold what was the original value for each '\0' character
	 * orig_values[i] will hold the original character between line i-1 and line i.
	 */
	char orig_values[MAX_NUMBER_CACHED_LINES - 1];

	/*
	 * an array that will hold the height and width of each line.
	 */
	SsdSize size_of_lines[MAX_NUMBER_CACHED_LINES];

	/*
	 * the number of lines which should be drawn
	 */
	int num_lines;

	/*
	 true iff the texts are cache
	 */
	BOOL is_text_cached; //   = FALSE;

	/*
	 true iff this text should be cached. For example if a text has a numnber of screen lines larger than
	 MAX_NUMBER_CACHED_LINES, this flag will be set to false, so we won't waste time
	 in the future trying to cache it in the future
	 */
	BOOL should_be_cached; // = TRUE;
...
static void init_containers(void) {
	...
	pen = roadmap_canvas_create_pen("ssd_text_pen", FALSE);
	roadmap_canvas_set_foreground(def_color);
#ifndef IPHONE
	cursor_image = (RoadMapImage) roadmap_res_get(RES_BITMAP, RES_SKIN,
			"cursor");
...
static void select_text_size(SsdWidget this) {
	...
	if (!this->value || !this->parent || !ctx->auto_size || ctx->auto_size_set)
		return;
...
static void reset_line_cache(SsdWidget widget) {
	int i;
	if (!widget)
		return;

	text_ctx_ptr ctx = (text_ctx_ptr) widget->data;
	if (!ctx->is_text_cached)
		return;
	ctx->is_text_cached = FALSE;
	for (i = 0; i < ctx->num_lines; i++) {
		if (i != 0) { // no line separator between before the first line...
			if (ctx->line_starts[i] - 1)
				*(ctx->line_starts[i] - 1) = ctx->orig_values[i - 1]; // switch back to original value between line i-1 and line i
			ctx->orig_values[i - 1] = '\0';
		}
		ctx->line_starts[i] = 0;
	}
	ctx->num_lines = 0;
...
static const char *get_value(SsdWidget widget) {
	text_ctx_ptr ctx = (text_ctx_ptr) widget->data;
	if (ctx->is_text_cached)
		reset_line_cache(widget);
	return widget->value;
}

static void format_cached_text(SsdWidget widget, int draw, RoadMapGuiRect *rect) {
	int i;
	int max_width = 0;
	RoadMapGuiPoint p;
	int text_height = 0;
	...
	for (i = 0; i < ctx->num_lines; i++) {
		if (draw) {
			p.y += ctx->size_of_lines[i].height;
			p.x = rect->minx;
			if (widget->flags & SSD_ALIGN_CENTER) {
				if (ssd_widget_rtl(NULL)) {
					p.x += (width - ctx->size_of_lines[i].width) / 2;
				} else {
					p.x += (width - ctx->size_of_lines[i].width) / 2;
				}
		...
		if (ctx->size_of_lines[i].width > max_width)
			max_width = ctx->size_of_lines[i].width;
		text_height += ctx->size_of_lines[i].height;
	}
	if ((rect->maxx = rect->minx + max_width - 1) < rect->maxx)
		rect->maxx = rect->minx + max_width - 1;
	rect->maxy = rect->miny + text_height * HEIGHT_FACTOR - 1;
}

static int format_text(SsdWidget widget, int draw, RoadMapGuiRect *rect) {
	...
	if (((widget->cached_size.height == -1)
			|| (widget->cached_size.width == -1)) && (ctx->is_text_cached)) { // size was reseted, calculate again
		reset_line_cache(widget);
	}

	if (ctx->is_text_cached) {
		format_cached_text(widget, draw, rect); //cached specific function
	} else {
		if (draw) {
			p.x = rect->minx;
			p.y = rect->miny;
		...
		if ((widget->flags & SSD_TEXT_PASSWORD) && (widget->value[0] != 0)) {
			text = "*****";
			ctx->should_be_cached = FALSE; // don't perform caching on this widget
		} else {
			text = widget->value;
		}

		if (ctx->should_be_cached)
			ctx->line_starts[0] = text;
		...
		while (*text || *line) {
			...
			roadmap_canvas_get_formated_text_extents(line, ctx->size,
					&text_width, &text_ascent, &text_descent, NULL, font_type);
			...
			if (!*text || new_line || (text_width > width)) {
				...
				if ((text_width > width) && (new_len > len)) {
					...
					roadmap_canvas_get_formated_text_extents(line, ctx->size,
							&text_width, &text_ascent, &text_descent, NULL,
							font_type);
				}
				...
				if (text_width > width && new_len )
				{
					...
					while (text_width > width  && new_len )
					{
						...
						roadmap_canvas_get_formated_text_extents(
								&line[display_offset], ctx->size,
								&estimated_width, &letter_ascent,
								&letter_descent, NULL, font_type);
				...
				if (draw) {
					...
					if ((text_height + h) < height) {
						roadmap_canvas_draw_formated_string_angle(&p, NULL, 0,
								ctx->size, font_type, &line[display_offset]);
					}
				}

				if (ctx->should_be_cached) {
					if (num_line < MAX_NUMBER_CACHED_LINES) {
						//cache the information for this line
						ctx->size_of_lines[num_line].height = h;
						ctx->size_of_lines[num_line].width = text_width;
						if ((num_line < MAX_NUMBER_CACHED_LINES - 1) && (*text)) {
							ctx->line_starts[num_line + 1] = text;
							ctx->orig_values[num_line] = *(text - 1); // the separator between the last line and this one
							*(text - 1) = '\0'; // insert null character, for future drawing
						}
						num_line++;
						ctx->num_lines = num_line;
						ctx->is_text_cached = TRUE;
					} else {
						reset_line_cache(widget); // this text has too many lines for cache - undo changes.
						ctx->should_be_cached = FALSE;// undo changes
						ctx->is_text_cached = FALSE;
					}
				}
...
static void draw(SsdWidget widget, RoadMapGuiRect *rect, int flags) {
	...
	if (widget->parent && (widget->flags & SSD_TEXT_INPUT)) {
		RoadMapImage cursor_image = (RoadMapImage) roadmap_res_get(RES_BITMAP,
				RES_SKIN, "cursor");
		...
		cursor_point.x = ssd_widget_rtl(NULL) ? (rect->minx - cursor_width)
				: rect->maxx;
...
static int set_value(SsdWidget widget, const char *value) {
	...
	ctx->should_be_cached = TRUE; // new value set, should try to be cached.
	reset_line_cache(widget);
...
static BOOL on_editbox_closed(int exit_code, const char* value, void* context) {
	SsdWidget text_widget = (SsdWidget) context;
	if (dec_ok == exit_code) // set the text for this widget
		ssd_text_set_text(text_widget, value);

}

static BOOL ssd_text_on_key_pressed(SsdWidget this, const char* utf8char,
		uint32_t flags) {
	text_ctx_ptr ctx = (text_ctx_ptr) this->data;
	...
	if( KEY_IS_ESCAPE )
		return FALSE;// let escape exit the dialog, don't handle it in BB.
	else if (!KEY_IS_BACKSPACE ) {
		char new_value[255];
		if( this->value && this->value[0])
			sprintf(new_value, "%s%s",this->value,utf8char);
		else
			sprintf(new_value, "%s",utf8char);
		if ( flags & SSD_TEXT_PASSWORD)
			ShowEditbox( "", new_value, on_editbox_closed, this, EEditBoxPassword );
		else
			ShowEditbox( "", new_value, on_editbox_closed, this, EEditBoxStandard );
		return;
	}
	if( KEY_IS_BACKSPACE )
#else
	...
	if (USING_PHONE_KEYPAD) {
		...
		const char* translated_char =
				roadmap_phone_keyboard_get_multiple_key_value(this, utf8char,
						ctx->input_type, &value_was_replaced);
...
SsdWidget ssd_text_new(const char* name, const char* value, int size, int flags) {
	SsdWidget w;
...
int ssd_text_get_char_height(int size) {
	...
	if (!s_char_height || (s_last_size != size)) {
		...
		roadmap_canvas_get_formated_text_extents("aAbB19Xx8", size, &text_width,
				&text_ascent, &text_descent, NULL, FONT_TYPE_BOLD);
...
int ssd_text_get_char_width() {
	...
	if (!s_char_width) {
		...
		roadmap_canvas_get_formated_text_extents("W", -1, &text_width,
				&text_ascent, &text_descent, NULL, FONT_TYPE_BOLD);
