...
#ifdef RIMAPI
#define DIFF_BETWEEN_FONTS_SIZE 7
#else
#define DIFF_BETWEEN_FONTS_SIZE 10
#endif
...
static void navigate_bar_draw_exit (int exit, int offset) {
	...
	if ( roadmap_screen_get_background_run() )
   {
	...
	sprintf (text, "%d", exit);
	pos.y += offset;
	roadmap_canvas_create_pen ("nav_bar_pen", FALSE);
...
//#define SMALL_SIZE  16
   ...
   while ((num_lines < 0) || ((num_lines > 1) && (NavigatePanel->street_line2_pos == -1))) {
      ...
      if (end == text) {
         ...
         roadmap_log (ROADMAP_ERROR, "Can't align street in nav bar: %s",street);
         free(text);
         return;
      }
		*end = '\0';
...
static void navigate_bar_draw_time_to_destination () {
   ...
   if (ssd_widget_rtl(NULL)){
     if (text_width == -1){
        roadmap_canvas_get_text_extents
              (text3, font_size-DIFF_BETWEEN_FONTS_SIZE, &text_width, &text_ascent, &text_descent, NULL);
        text_width += 3;
      }
   ...
   if (!ssd_widget_rtl(NULL)){
    ...
    roadmap_canvas_draw_string_size(&position, ROADMAP_CANVAS_BOTTOMLEFT, font_size-DIFF_BETWEEN_FONTS_SIZE, text3);
...
static void navigate_bar_draw_distance_to_destination () {
   ...
   if (roadmap_skin_state() == 1)
  	...
  	if (ssd_widget_rtl(NULL)){
	   ...
	   roadmap_canvas_draw_string_size(&position, ROADMAP_CANVAS_BOTTOMLEFT, font_size-DIFF_BETWEEN_FONTS_SIZE, text);
      ...
      position = NavigatePanel->distance_to_destination_pos;
      position.x =roadmap_canvas_width() - distance_offset;
      roadmap_canvas_draw_string_size(&position, ROADMAP_CANVAS_BOTTOMRIGHT, font_size-DIFF_BETWEEN_FONTS_SIZE, text);
