...
static RoadMapConfigDescriptor NavigateConfigLocalCalcEnabled =
                        ROADMAP_CONFIG_ITEM( "Navigation", "LocalCalc enabled" );

static void set_last_nav_time ();
void navigate_log_state (void);
static BOOL showETA = FALSE;
static void switchETAtoTime(void);

static BOOL NavigateStateActive = FALSE;
...
INLINE_DEC NavigateSegment * navigate_segment (int i) {
...
static NavigateSegment * navigate_segment_func (int i) {

	return navigate_segment(i);
}
...
BOOL navigate_main_local_calc_enabled(){
   if (roadmap_config_match(&NavigateConfigLocalCalcEnabled,"yes"))
      return TRUE;
   else
      return FALSE;
}
   ...
   *from_point = -1;
   ...
   if ((roadmap_navigate_retrieve_line
      ...
      //roadmap_messagebox ("Error", "Can't find a road near destination point.");
		roadmap_log (ROADMAP_ERROR, "Failed to find a valid road near destination %d,%d", position->longitude, position->latitude);
      to_line->plugin_id = -1;
      *to_point = 0;
...
INLINE_DEC void navigate_get_plugin_line (PluginLine *line, const NavigateSegment *segment) {
...
static void navigate_main_on_segments (NavigateRouteRC rc, const NavigateRouteResult *res, const NavigateRouteSegments *segments) {
   ...
   CalculatingRoute = FALSE;
	...
	{
		roadmap_log( ROADMAP_ERROR, "Navigation status was not supplied!" );
	}
...
static int navigate_main_recalc_route (int delay_message) {
   ...
   if (navigate_find_track_points_in_scale
	...
	if (roadmap_plugin_same_line(&from_line, &NavigateFromLineLast) &&
		...
		roadmap_log (ROADMAP_ERROR, "Trying to recalc from same line at %d/%d", from_line.square, from_line.line_id);
		return -1;
	}
   ...
   if (NavigateIsByServer &&
	   ...
	   navigate_cost_reset ();
   	roadmap_log (ROADMAP_ERROR, "Calculating short reroute..");
	   track_time =
	...
	if (track_time < 0) {
	   if (RealTimeLoginState () /*&& (!NavigateIsAlternativeRoute)*/) {
   		...
   		roadmap_log (ROADMAP_ERROR, "Requesting reroute..");
	   ...
	   } else if (navigate_main_local_calc_enabled()){
	   	NavigateIsByServer = 0;
   		NavigateNumOutlinePoints[0] = 0;
		   navigate_cost_reset ();
   		roadmap_log (ROADMAP_ERROR, "Calculating long reroute..");
		   track_time =
		      navigate_route_get_segments
		            (&from_line, from_point, &NavigateDestination, &NavigateDestPoint,
		   ...
		   roadmap_log (ROADMAP_ERROR, "LocalCalc disabled- Aborting long reroute..");
		   track_time= -1;
	   }
	}
   ...
   navigate_instr_prepare_segments (navigate_segment_func, navigate_num_segments (), num_new,
                                   &NavigateSrcPos, &NavigateDestPos);
...
void navigate_get_next_line
   ...
   if ((NavigateCurrentSegment < num_segments) &&
      ...
      roadmap_log (ROADMAP_INFO, "Recalculating route...");
...
void navigate_main_initialize (void) {
...
#ifndef J2ME
   roadmap_config_declare_enumeration
      ("preferences", &NavigateConfigLocalCalcEnabled, NULL, "yes", "no", NULL);
#else
   roadmap_config_declare_enumeration
      ("preferences", &NavigateConfigLocalCalcEnabled, NULL, "no", "yes", NULL);
#endif// J2ME
   ...
   if (roadmap_config_get_integer (&NavigateConfigNavigating) &&
   	...
   	roadmap_trip_remove_point ("Destination");
      roadmap_config_set_integer (&NavigateConfigNavigating, 0);
      roadmap_config_save_one ("session",1);
   }
...
int navigate_main_calc_route (void) {
   ...
   if(LOG_NAVIGATION_STATE && !NavigateStateActive){
  	   NavigateStateActive = TRUE;
   	   roadmap_main_set_periodic(LOG_NAVIGATION_TIMEOUT,navigate_log_state);
   }
   ...
   if (RealTimeLoginState ()) {
   	...
   	roadmap_log (ROADMAP_ERROR, "Requesting new route..");
   	...
   	roadmap_log (ROADMAP_ERROR, "Calculating new route..");
	   track_time =
	      navigate_route_get_segments
	            (&from_line, from_point, &NavigateDestination, &NavigateDestPoint,
	   ...
	   roadmap_log (ROADMAP_ERROR, "LocalCalc disabled- Aborting new route..");
	   track_time= -1;
   }
   ...
   if (track_time <= 0) {
      ...
      NavigateDetourSize = 0;
      NavigateDetourEnd = 0;
      navigate_instr_prepare_segments (navigate_segment_func, num_segments, num_new_segments,
                                      &NavigateSrcPos, &NavigateDestPos);
...
timeStruct curTimeOnPhone;
   ...
   roadmap_time_get_time(&timeBuffer);
   hours = timeBuffer.hours;
   minutes = timeBuffer.minutes;
   seconds = timeBuffer.seconds;
...
void navigate_log_state (void) {
   ...
   roadmap_log (ROADMAP_ERROR, "NavigateEnabled = %d CalculatingRoute = %d ReCalculatingRoute = %d NavigateOfftrackTime = %d NavigateRerouteTime = %d",
		   NavigateEnabled, CalculatingRoute, ReCalculatingRoute,(int)(NavigateOfftrackTime - now),
		   (int)(NavigateRerouteTime - now) );

   roadmap_log (ROADMAP_ERROR,"NavigateNumSegments = %d NavigateNumInstSegments = %d NavigateCurrentSegment = %d NavigateDetourSize = %d NavigateDetourEnd = %d NavigateIsByServer = %d",
		   NavigateNumSegments, NavigateNumInstSegments, NavigateCurrentSegment, NavigateDetourSize , NavigateDetourEnd, NavigateIsByServer);
}
