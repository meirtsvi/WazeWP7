...
CIBYL_EXPORT_SYMBOL(roadmap_gpsj2me_pos1);
CIBYL_EXPORT_SYMBOL(roadmap_gpsj2me_pos2);

void roadmap_gpsj2me_pos2 (int status, int time, int speed, int azymuth) {
   RoadmapGpsJ2meNavigationListener
         ((char)status, time, data.latitude, data.longitude, ROADMAP_NO_VALID_DATA, speed, azymuth);
}

void roadmap_gpsj2me_pos1 (int latitude, int longitude, int altitude) {
   data.latitude = latitude;
   data.longitude = longitude;
//   data.altitude = ROADMAP_NO_VALID_DATA;
}
