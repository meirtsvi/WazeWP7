...
static void on_conf_file_downloaded (const char* res_name, int success, void *context, char *last_modified) {
   ...
   if (!prompt_set_exist(system_prompts)){
      ...
      system_prompts=roadmap_prompts_get_name();
#ifdef J2ME  // J2ME doesn't come installed with english prompts, so download.
      roadmap_prompts_download("eng");
#endif
   }
...
void roadmap_prompts_init (void) {
   ...
   if (prompt[0] == 0) {
	  const char * lang = strdup(roadmap_lang_get_system_lang ());
	  char* temp = strstr(lang,"_heb");
      if(temp){
    	  *temp = '\0'; // change eng_heb to eng, since eng_heb prompt isn't defined.
      }
      roadmap_prompts_set_name (lang);
      free(lang);
   }
...
/*   BOOL exist;
   ...
   exist = roadmap_file_exists (path, "within.bin");
#else
   exist = roadmap_file_exists (path, "within.mp3");
#endif
   return exist;*/
}
