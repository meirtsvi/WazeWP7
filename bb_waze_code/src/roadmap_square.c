...
static int roadmap_square_cache (int square) {
	...
	{
      roadmap_log(ROADMAP_INFO,"SQUARE cache low - removing squares(%d)!!!!\n", slot);
...
int roadmap_square_current_scale_shift (void) {

	if (!RoadMapSquareActive) {
		return 1;
	}

	return roadmap_tile_get_scale_shift (RoadMapSquareActive->Square[RoadMapSquareCurrentSlot]->square->scale);
}
