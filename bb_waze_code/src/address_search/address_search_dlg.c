...
{
   if( (new_value != NULL )&& (!strcmp(new_value, roadmap_lang_get(COULDNT_FIND_ADDRESS_TEXT))))
   {
      send_error_report();
...
static void on_search(void)
{
   ...
   if ( !strcmp( "##@esp", ssd_text_get_text( edit ) ) )
   {
      roadmap_lang_set_system_lang("espanol", NULL);
      roadmap_messagebox("", "Language changed to Spanish, please restart waze");
      return;
   }
...
int on_options(SsdWidget widget, const char *new_value, void *context)
{
   ...
   ssd_contextmenu_show_item( &context_menu,
                              cm_send,
                              b_report_wrong_address&&!generic_search_dlg_is_1st(search_address),
                              FALSE);
   ...
   s_menu = ssd_context_menu_show(  menu_x,              // X
