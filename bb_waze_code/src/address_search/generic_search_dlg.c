...
void generic_search_dlg_switch_gui(void)
{
   ...
   {
#if (defined(__SYMBIAN32__) && !defined(TOUCH_SCREEN)) || defined(ANDROID) || defined(RIMAPI)
    ssd_dialog_draw ();
...
void reopen_keyboard(void)
{
   ...
   ShowEditbox(roadmap_lang_get( search_context[s_type].c_dlg_title), search_context[s_type].c_saved_txt,
            on_keyboard_closed, NULL, EEditBoxStandard | EEditBoxEmbedded | EEditBoxActionSearch);
#endif
...
#endif
#ifdef RIMAPI
   ssd_contextmenu_menu_button_register(left_sk_callback,dlg_name);
#endif
...
#ifdef TEST_EDITBOX
static SsdKeyboardCallback editbox_cb;
static void *editbox_context;
static int editbox_rc= 0;
static void box_closed_cb( int exit_code ) {
	printf("exit %d\n",editbox_rc);
	if (editbox_cb) {
		editbox_cb(editbox_rc %2,editbox_rc<2 ? "raanana" : "",editbox_context);
	}
	editbox_cb= 0;
	editbox_context= 0;
}
void ShowEditbox_t(const char* aTitleUtf8, const char* aTextUtf8, SsdKeyboardCallback callback,
					  void *context, TEditBoxType aBoxType ){
	editbox_cb= callback;
	editbox_context= context;
	++editbox_rc;
	if (editbox_rc==4)
		editbox_rc= 0;
	char title[32];
	snprintf(title,sizeof title,"%s=%d,%d",aTitleUtf8,editbox_rc,aBoxType);
	title[sizeof title -1]= '\0';
	roadmap_messagebox_cb(title, aTextUtf8, box_closed_cb);
}
#endif// TEST_EDITBOX
