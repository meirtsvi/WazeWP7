...
void roadmap_message_set (char parameter, const char *format, ...) {
    ...
    */
    ...
    if (RoadMapMessageParameters[(int)parameter] == NULL) {
        RoadMapMessageParameters[(int)parameter] = malloc(100);
    }

    va_start(ap, format);
    vsnprintf(RoadMapMessageParameters[(int)parameter], 100, format, ap);
    RoadMapMessageParameters[(int)parameter][99] = '\0';
    va_end(ap);
}
...
void roadmap_message_unset (char parameter) {
    ...
    if (RoadMapMessageParameters[(int)parameter] != NULL) {
        RoadMapMessageParameters[(int)parameter][0] = '\0';
    }
}
...
int roadmap_message_is_set (char parameter) {
    ...
    return ((RoadMapMessageParameters[(int)parameter] != NULL) &&
               RoadMapMessageParameters[(int)parameter][0]);
}
