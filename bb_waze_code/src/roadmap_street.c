...
#ifdef J2ME
	roadmap_line_fast_points (line, &neighbour->from, &neighbour->to);
#else
...
(RoadMapNeighbour *neighbours, int count, int max,
	...
	for (slot = count; slot > 0 && neighbours[slot - 1].distance > this->distance; slot--);
	...
	/* check for duplicate */
	for (i = slot - 1; (i >= 0) && (neighbours[i].distance == this->distance); i--)
		if (neighbours[i].from.longitude == this->from.longitude &&
				...
				roadmap_plugin_same_line (&neighbours[i].line, &this->line)) {
			return count;
		}

	if ((slot < count) && (slot != (max - 1))) {
		/* make room in the arrary */
		int move_count = count - slot;
		if (count == max) move_count--;

		memmove(neighbours + slot + 1, neighbours + slot,
				sizeof(neighbours[0]) * move_count);
	}

	memcpy(neighbours + slot, this, sizeof(neighbours[0]));
...
static const char *roadmap_street_get_street_name_from_id (int street) {
	...
	roadmap_street_get_street_name_from_id_safe(street, RoadMapStreetName, sizeof(RoadMapStreetName));
	return RoadMapStreetName;
}


void roadmap_street_get_street_name_from_id_safe (int street, char *name, int size) {
