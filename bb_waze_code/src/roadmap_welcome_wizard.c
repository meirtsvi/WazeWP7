...
#ifdef RIMAPI
static void rim_terms_of_use_accepted();
static void rim_terms_of_use_declined(){
	roadmap_log(ROADMAP_ERROR,"User declined disclaimer");
	roadmap_main_exit();
}
#endif
// Context menu:
...
#ifdef RIMAPI
   SSD_CM_INIT_ITEM  ( "Personalize",    ww_personalize),
#endif// RIMAPI
...
static char * DISCLAIMER_ESP[NUMBER_OF_DISCLAIMER_PARTS][NUMBER_OF_LINES_PER_PART_DISC] =
				{{ DIS_ESP_0_0,"","","","",""},{DIS_ESP_1_0,DIS_ESP_1_1,DIS_ESP_1_2,DIS_ESP_1_3,
...
static char * INTRO_ESP[NUMBER_OF_LINES_INTRO] = {INTR_ESP_0,INTR_ESP_1,INTR_ESP_2,INTR_ESP_3,INTR_ESP_4};
...
void welcome_wizard_twitter_dialog(void){
   ...
   group = ssd_container_new ("Twitter Name group", NULL,
                               SSD_MAX_SIZE,ssd_container_get_txtbox_height(),
...
static void workaddress_dialog(){
   ...
   int txt_box_height = ssd_container_get_txtbox_height();
...
#else //TOUCH_SCREEN
   ssd_widget_set_right_softkey_text       ( dialog, roadmap_lang_get("Skip"));
   ssd_widget_set_right_softkey_callback   ( dialog, on_skip_welcome);

   ssd_widget_set_left_softkey_text      ( dialog, roadmap_lang_get("Personalize"));
   ssd_widget_set_left_softkey_callback  ( dialog, on_personalize);
#endif //TOUCH_SCREEN
   ssd_widget_add (group, space(20));
   ssd_widget_add (dialog, group);
...
static SsdWidget add_text_to_group(const char * in_text,SsdWidget group, int tab_flag, int font_size, BOOL first_widget,
	...
	SsdWidget text;
	if(first_widget)
		text = ssd_text_new( text_name, "", font_size, tab_flag|SSD_WS_DEFWIDGET);
	else
		text = ssd_text_new( text_name, "", font_size, tab_flag);
...
static void add_intro_texts(SsdWidget group, int tab_flag){
	...
	for (i=0;i<NUMBER_OF_LINES_INTRO;i++){
		if (!strcmp(intro_text[i],"")){// reached an empty screen - got to the last available text
			...
			sprintf(text_name,"welcome_intro_text_%d",i);
			last_widget = add_text_to_group(intro_text[i],group,tab_flag,font_size,first_widget,text_name);
			first_widget = FALSE;
		}
	}
...
#ifdef RIMAPI
	i = 1; // to make the disclaimer screen smaller, skip the "intro" texts
#endif 	
   ...
   for (;i<NUMBER_OF_DISCLAIMER_PARTS;i++){
   		for (j=0;j<NUMBER_OF_LINES_PER_PART_DISC;j++){
	   		if (!strcmp(disclaimer[j+i*NUMBER_OF_LINES_PER_PART_DISC],"")){// reached an empty screen - got to the last available text
				...
				if( ((i==0) && ( j != 0 )))
					flag = 0;  // don't add tab_stop to the small texts, they're shown anyway. 
				else
					flag = tab_flag;
				sprintf(text_name,"welcome_disc_text_%d.%d",i,j);
				last_widget = add_text_to_group(disclaimer[j+i*NUMBER_OF_LINES_PER_PART_DISC],group,flag,font_size, first_widget,text_name);
				first_widget = FALSE;
			}
   		}
   }
...
#ifdef RIMAPI // in RIM add the buttons, in addition to the soft keys
   ssd_widget_add (group,
   ssd_button_label ("Accept", roadmap_lang_get ("Accept"),
                     SSD_WS_TABSTOP|SSD_ALIGN_CENTER|SSD_START_NEW_ROW, terms_of_use_accepted));

   ssd_widget_add (group,
   ssd_button_label ("Decline", roadmap_lang_get ("Decline"),
                     SSD_WS_TABSTOP|SSD_ALIGN_CENTER, terms_of_use_declined));
#endif
...
#else //TOUCH_SCREEN
   ssd_widget_set_right_softkey_text       ( dialog, roadmap_lang_get("Options"));
   ssd_widget_set_right_softkey_callback   ( dialog, on_options);

#ifdef RIMAPI
      ssd_contextmenu_menu_button_register(on_options,dialog->name);
#endif
...
static const char* guided_tour_url(void)
{
   ...
   snprintf( s_url, sizeof(s_url), "%s?deviceid=%d&width=%d&height=%d",
         roadmap_config_get( &WELOCME_WIZ_GUIDED_TOUR_URL_Var ), RT_DEVICE_ID,
         roadmap_canvas_width(), roadmap_canvas_height() - roadmap_bar_bottom_height() );
...
#if defined(RIMAPI)
   NOPH_ConfirmDialog_CreateDialog("",&text[0], 1, roadmap_lang_get("Accept"), roadmap_lang_get("Decline"),0,rim_terms_of_use_accepted, rim_terms_of_use_declined,0);
#endif // RIMAPI
