...
extern RoadMapShape* RoadMapShapeActiveShape;

extern int shape_cache_square;
extern int shape_cache_scale_factor;
extern int shape_cache_scale_shift;
...
INLINE_DEC void roadmap_shape_get_position (int shape, RoadMapPosition *position) {
   ...
   position->longitude += (int)RoadMapShapeActiveShape[shape].delta_longitude * shape_cache_scale_factor;
   position->latitude  += (int)RoadMapShapeActiveShape[shape].delta_latitude * shape_cache_scale_factor;
}


INLINE_DEC void roadmap_shape_get_position_fast (int shape, RoadMapPosition *position) {

   int t = *(int *)(&RoadMapShapeActiveShape[shape].delta_longitude);
   
   position->longitude += (t >> 16) << shape_cache_scale_shift;
   position->latitude  += (t << 16) >> (16 - shape_cache_scale_shift);
}
...
#ifndef J2ME
   return RoadMapShapeActiveShape[shape].delta_latitude;
#else
   return (*(int *)&RoadMapShapeActiveShape[shape].delta_longitude);
#endif
}
...
void roadmap_shape_get_position_fast (int shape, RoadMapPosition *position);
int roadmap_shape_get_count (int shape);
int roadmap_shape_count (void);
void roadmap_shape_set_square (void);
