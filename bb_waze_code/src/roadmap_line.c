...
RoadMapLine *RoadMapLineActiveLine = NULL;
...
static void roadmap_line_activate (void *context) {
   ...
   RoadMapLineActiveLine = line_context->Line;
}
...
int  roadmap_line_from_is_fake (int line) {
	...
	return RoadMapLineActiveLine[line].from & POINT_FAKE_FLAG;
}


int  roadmap_line_to_is_fake (int line) {

	return RoadMapLineActiveLine[line].to & POINT_FAKE_FLAG;
}
...
int roadmap_line_get_range (int line) {
   ...
   if (line < 0 || line >= RoadMapLineActive->LineCount) {
	...
	range = RoadMapLineActiveLine[line].range;
...
void roadmap_line_points (int line, int *from, int *to) {
   ...
   *from = RoadMapLineActiveLine[line].from & POINT_REAL_MASK;
   *to = RoadMapLineActiveLine[line].to & POINT_REAL_MASK;
}
...
void roadmap_line_point_ids (int line, int *id_from, int *id_to) {
   ...
   *id_from = roadmap_point_db_id (RoadMapLineActiveLine[line].from & POINT_REAL_MASK);
   *id_to = roadmap_point_db_id (RoadMapLineActiveLine[line].to & POINT_REAL_MASK);
}
