...
CIBYL_EXPORT_SYMBOL(roadmap_gps_coarse_fix);
#endif
...
#ifdef RIMAPI
static RoadMapConfigDescriptor RoadMapConfigGPSGearsWebServiceURL =
                        ROADMAP_CONFIG_ITEM("GPS", "Gears WebService url");


/*
 * This config determines if we will search for a cell id through the google gears API
 * which converts cell tower information to longitutde and latitude. If anytime in the future
 * google disable this feature, then server should send clients a signal that this
 * config needs to be disabled.
 */
static RoadMapConfigDescriptor RoadMapConfigCellIdEnabled =
      ROADMAP_CONFIG_ITEM("GoogleCellId", "enabled");
#endif
...
#ifdef RIMAPI // search fo cell id
   if (!strcmp (roadmap_config_get (&RoadMapConfigCellIdEnabled), "yes")){
	   NOPH_GpsManager_getCoarseFix(roadmap_config_get (&RoadMapConfigGPSGearsWebServiceURL));
   }
#endif
...
#ifdef RIMAPI
      roadmap_config_declare
      	  ("preferences", &RoadMapConfigGPSGearsWebServiceURL, "http://www.google.com/loc/json", NULL);

      roadmap_config_declare_enumeration ("preferences", &RoadMapConfigCellIdEnabled, NULL, "yes",
                     "no", NULL);
      if (!strcmp (roadmap_config_get (&RoadMapConfigCellIdEnabled), "yes")){
    	  NOPH_GpsManager_getCoarseFix(roadmap_config_get (&RoadMapConfigGPSGearsWebServiceURL));
      }
...
// This is T E R R I B L E  !!! We have to redesign this!! AGA
   ...
   if (1) {
      ...
      if (!strchr(url, ':')) {
         ...
         if (NOPH_GpsManager_connect (gps_mgr, url) != -1) {
            RoadMapGpsLink.subsystem = ROADMAP_IO_NULL;
   ...
   switch (RoadMapGpsProtocol) {
      ...
      case ROADMAP_GPS_CSV:
         ...
         break;
#endif //CSV_GPS
#if !defined (J2ME) && !defined (__SYMBIAN32__) && !defined (ANDROID)
