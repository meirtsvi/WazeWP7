...
extern RoadMapPoint *RoadMapPointActivePoint;
...
/* point is already masked with POINT_REAL_MASK */
INLINE_DEC void roadmap_point_position_safe
                     (int point, RoadMapPosition *position) {

   RoadMapPoint *Point;

   Point = RoadMapPointActivePoint + point;
...
#ifdef J2ME
   Point = RoadMapPointActivePoint + point_id;
   position->longitude = Point->longitude;
   position->latitude  = Point->latitude;
