...
static RoadMapConfigDescriptor   RoadMapConfigForceLanguage   =
            ROADMAP_CONFIG_ITEM("GeoConfig", "Force Language");            
...
void roadmap_geo_config_failed_cb(int exit_code );
...
static void retreis_exhausted(void){
   ...
   if ( roadmap_screen_is_hd_screen() && (roadmap_canvas_height() > hd_height*4) )
   {
	   height = hd_height;
   }
...
#ifndef J2ME TODODAN
   download_lang_files(); // in j2me don't download all the lang files
#endif
...
void roadmap_geo_config (RoadMapCallback callback) {
   ...
   force_lang = roadmap_config_get( &RoadMapConfigForceLanguage);
   //set the language
   if (strcmp(force_lang, "")){ // language should be forced
     roadmap_lang_set_system_lang (force_lang, NULL);
   }

   roadmap_log(ROADMAP_ERROR, "roadmap_geo_config - %s", force_location);
   if (need_to_ask_server () && *force_location){
      ...
      if (!strcmp(force_location, "il")){
         roadmap_log(ROADMAP_ERROR, "roadmap_geo_config - force location set to il");
         roadmap_geo_config_il(callback);
         return;
      }
      else if (!strcmp(force_location, "usa")){
         roadmap_log(ROADMAP_ERROR, "roadmap_geo_config - force location set to usa");
         roadmap_geo_config_usa(callback);
         return;
      }
      else if (!strcmp(force_location, "other")){
      	 roadmap_log(ROADMAP_ERROR, "roadmap_geo_config - force location set to other");
         roadmap_geo_config_other(callback);
         return;
      }
      else{
         roadmap_log(ROADMAP_ERROR, "roadmap_geo_config - force location invalide value %s, continuing nromally", force_location);
      }
   }
