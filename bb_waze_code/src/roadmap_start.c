...
#ifdef RIMAPI
static void shortcuts_dialog(char *about){
...
static void roadmap_start_about_exit (void) {
   ...
   char about[500];
   sprintf (about, "%s\n%s",
            roadmap_lang_get("Are you sure you want to"),
            roadmap_lang_get("turn off waze?"));
   about_exit_dialog(about);
}
...
void roadmap_start_login_cb (void){
   if (0) { //put back as soon as i have proper dialog boxes RoadMapStartAfterCrash) {
...
#ifdef J2ME
    time_t t = NOPH_System_currentTimeMillis();
    printf("%d - In roadmap_start!\n", t);
#endif
#ifdef ROADMAP_DEBUG_HEAP
...
#ifndef J2ME
   /* Too slow due to realtime traffic deletes - bring back when using pools */
