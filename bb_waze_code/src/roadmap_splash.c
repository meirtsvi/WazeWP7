...
static SplashFiles RoadMapSplashFiles[] = {
   ...
   {"welcome_360_640", 360, 640,FALSE},
   {"welcome_360_480", 360, 480,FALSE},
   {"welcome_360_400", 360, 400,FALSE},
   ...
   {"welcome_wide_480_360", 480,360, TRUE},
...
static void download_splash(void){
   ...
   roadmap_log(ROADMAP_ERROR,"tried downloading file_name : %s \n", file_name);
   if (!file_name)
      return;
...
static void download_wide_splash(void){
   ...
   roadmap_log(ROADMAP_ERROR,"tried downloading wide file_name : %s \n", file_name);
...
void roadmap_splash_display (void) {
   ...
   if (height > width)
      image = (RoadMapImage) roadmap_res_get(RES_BITMAP, RES_SKIN|RES_NOCACHE|RES_DOWNLOADED_IMAGE, "welcome");
   else
      image = (RoadMapImage) roadmap_res_get(RES_BITMAP, RES_SKIN|RES_NOCACHE|RES_DOWNLOADED_IMAGE, "welcome_wide");

   if( !image)
      return;

   pos.x = 0;//(width - roadmap_canvas_image_width(image)) / 2;
   pos.y = 0;//(height - roadmap_canvas_image_height(image)) / 2;
