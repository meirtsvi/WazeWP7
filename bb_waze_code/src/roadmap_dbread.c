...
#ifdef RIMAPI
	status = NOPH_ZLib_uncompress (raw_data, &raw_data_size, compressed_data, tile_header->compressed_data_size);
#else
	status = uncompress (raw_data, &raw_data_size, compressed_data, tile_header->compressed_data_size) == Z_OK;
#endif

	if (!status) {
	   roadmap_log (ROADMAP_ERROR, "data file open: uncompress failed - size:%d", size);
		free (raw_data);
		return 0;
	}
