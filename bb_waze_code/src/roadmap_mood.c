...
#ifdef J2ME
#define roadmap_path_join(str,mood) mood
#endif
   switch(mood)
   {
...
#ifdef J2ME
#undef roadmap_path_join(str,mood)
#endif
}
...
#else // j2me
	static char * moods [] = {
			MOOD_Name_LOL, MOOD_Name_LOL_Female , MOOD_Name_Bored, MOOD_Name_Bored_Female,
			MOOD_Name_Busy, MOOD_Name_Busy_Female, MOOD_Name_Cool, MOOD_Name_Cool_Female,
			MOOD_Name_Happy, MOOD_Name_Happy_Female, MOOD_Name_In_A_Hurry, MOOD_Name_In_A_Hurry_Female,
			MOOD_Name_InLove, MOOD_Name_InLove_Female, MOOD_Name_Mad, MOOD_Name_Mad_Female,
			MOOD_Name_Peaceful, MOOD_Name_Peaceful_Female, MOOD_Name_Sad, MOOD_Name_Sad_Female,
			MOOD_Name_Singing, MOOD_Name_Singing_Female, MOOD_Name_Sleepy, MOOD_Name_Sleepy_Female,
			MOOD_Name_Speedy, MOOD_Name_Speedy_Female, MOOD_Name_Starving, MOOD_Name_Starving_Female,
			MOOD_Name_Wondering, MOOD_Name_Wondering_Female};
		 
	for (i = 0; i < sizeof(moods)/sizeof(char *);i++){
		labels[i] = moods[i];
		count ++;
	}
#endif
