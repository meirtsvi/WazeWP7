...
static void j2me_sort_alerts(comparison_fn_t compare_alerts) {
  ...
  for (k = 1; k < gExternalPoisDisplayList.iCount; ++k) {
	 int temp_ent= gExternalPoisDisplayList.entitiesID[k];
    int i = k - 1;
    push_alert_to_front = ((*compare_alerts)(&temp_ent, &gExternalPoisDisplayList.entitiesID[i] ) < 0);
    while ((i >= 0) && push_alert_to_front ) {
    	gExternalPoisDisplayList.entitiesID[i + 1] = gExternalPoisDisplayList.entitiesID[i];
       --i;
       if(i>=0)
      	 push_alert_to_front = ((*compare_alerts)(&temp_ent, &gExternalPoisDisplayList.entitiesID[i] ) < 0);
    }
    gExternalPoisDisplayList.entitiesID[i + 1] = temp_ent;
  }
}
#endif// J2ME
...
#else
   j2me_sort_alerts(RealtimeExternalPoi_DisplayList_Compare);
#endif
