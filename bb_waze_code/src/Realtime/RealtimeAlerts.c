...
static int isAlertCountTimerRedrawCBOn = FALSE;
#ifdef J2ME
typedef int (*comparison_fn_t) (__const void *, __const void *);
static void j2me_sort_alerts(comparison_fn_t compare_alerts);
#endif// J2ME

#define RTAlertsCountsRedrawMiliSeconds 2000
...
// The number of alerts has changed - update screen. This is set as a timer, so we won't
// redraw after every alert add/remove, but maximum once every RTAlertsCountsRedrawMiliSeconds
static int alertCountTimerRedrawCB(){
	roadmap_screen_redraw();
	roadmap_main_remove_periodic(alertCountTimerRedrawCB) ;
	isAlertCountTimerRedrawCBOn = FALSE;
}
...
BOOL RTAlerts_Add(RTAlert *pAlert)
{
   ...
   if (pAlert->bArchive)
    ...
    if(!isAlertCountTimerRedrawCBOn){
   	 isAlertCountTimerRedrawCBOn = TRUE;
   	 roadmap_main_set_periodic(RTAlertsCountsRedrawMiliSeconds,alertCountTimerRedrawCB);
    }
    return TRUE;
}
...
#else
		j2me_sort_alerts(compare_proximity);
#endif
    }
    else {
#ifndef J2ME
...
#else
		j2me_sort_alerts(compare_recency);
#endif
    }
}
...
static void RTAlerts_popup_alert(int alertId, int iCenterAround)
{
   ...
   ssd_widget_add( position_con, spacer );
    ...
    flags = SSD_ROUNDED_BLACK|SSD_POINTER_LOCATION;
#if 0// RIMAPI
    flags = flags|SSD_DIALOG_FLOAT_MAX_WIDTH;
#endif
    popup = ssd_popup_new("AlertPopUPDlg", "", on_popup_close, SSD_MAX_SIZE, SSD_MIN_SIZE,&position, flags);
                                 ...
                                 void*       context)
{
    ...
    //GetEditBoxCameraImagePath(&gCurrentImagePath);
#endif// RIMAPI_OS_VER>= 5
#endif
    if ( gCurrentImagePath ){
    	roadmap_log(ROADMAP_WARNING,"uploading image %s",gCurrentImagePath);
...
void add_real_time_chit_chat()
{
   ...
   if (Realtime_AnonymousMsg()) {
    ...
    AlertConext->szGroup = "";
#if (defined(__SYMBIAN32__) && !defined(TOUCH_SCREEN)) || (defined (RIMAPI) && RIMAPI_OS_VER>= 5) || defined (IPHONE) || defined (ANDROID)
    ShowEditboxCamera(roadmap_lang_get("Chat"), "", roadmap_lang_get("Add image"), roadmap_lang_get("Send"), roadmap_lang_get("Change image"),
            on_keyboard_closed, AlertConext, EEditBoxEmptyForbidden |EEditBoxStandard );
#else
...
#if (defined(__SYMBIAN32__) && !defined(TOUCH_SCREEN)) || defined (RIMAPI) || defined (IPHONE) || defined (ANDROID)
    ShowEditbox(roadmap_lang_get("Comment"), "", post_comment_keyboard_callback,
            pAlert, EEditBoxEmptyForbidden |EEditBoxStandard );
#else
...
/*
 *  Insertion sort implementation to sort alerts, since qsort not supported in j2me.
 *  The array is small, max 40 - Insertion Sort run just as fast. ( probably faster actually ).
 */
static void j2me_sort_alerts(comparison_fn_t compare_alerts) {
  int k;
  BOOL push_alert_to_front ; // will be true while we still need to switch in a certain loop.
  for (k = 1; k < gAlertsTable.iCount; ++k) {
	 RTAlert * temp_alert= gAlertsTable.alert[k];
    int i = k - 1;
    push_alert_to_front = ((*compare_alerts)(&temp_alert, &gAlertsTable.alert[i] ) < 0);
    while ((i >= 0) && push_alert_to_front ) {
   	 gAlertsTable.alert[i + 1] = gAlertsTable.alert[i];
       --i;
       if(i>=0)
      	 push_alert_to_front = ((*compare_alerts)(&temp_alert, &gAlertsTable.alert[i] ) < 0);
    }
    gAlertsTable.alert[i + 1] = temp_alert;
  }
}
#endif// J2ME
