...
#if defined (_WIN32) || defined (__SYMBIAN32__) || defined(RIMAPI)
...
void UpgradeVersion()
{
   const char* szTitle  = "Update Waze!";
   const char* szText   = "";
   ...
   switch( gs_VU.eSeverity)
   {
      ...
      case VUS_Low:
         szText = "A great new version is available. Would you like to install it now?";
         break;
   ...
   ssd_confirm_dialog      (  roadmap_lang_get(szTitle),
                              roadmap_lang_get(szText),
                              TRUE,
...
void OnTransactionCompleted( void* ctx, roadmap_result rc)
{
   ...
   switch( gs_CI.LastError)
   {
      ...
      case err_rt_wrong_name_or_password:
      {
    	 ...
    	 {
			 if(!roadmap_login_is_login_active()){
...
#define  MAP_DRAG_EVENT__TERMINATION_THRESHOLD     (5)   // Seconds
#else
#define  MAP_DRAG_EVENT__PEEK_INTERVAL             (1000) // Milliseconds
...
static void Realtime_SessionDetailsSave( void )
{
   ...
   roadmap_config_save_one("user", 1);
}
