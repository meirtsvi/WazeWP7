...
static void populate_list(){
    ...
    while ((iAlertCount > 0) && (iCount < MAX_ALERTS_ENTRIES))    {
        ...
        widget = ssd_container_new("list", "list_row", SSD_MAX_SIZE, SSD_MAX_SIZE, 0);
       ...
       if (alert->bArchive) {
        ...
        text = ssd_text_new("alert_txt", AlertStr, 16, 0);
        ...
        text_w = ssd_text_new ("Alert Distance", dist_str, 16, SSD_ALIGN_RIGHT|SSD_TEXT_NORMAL_FONT);
        ssd_widget_set_offset(text_w, 0, offset_y);
        ssd_widget_add(widget, text_w);

        ssd_dialog_add_hspace(widget, 5, SSD_ALIGN_RIGHT);
        text_w = ssd_text_new ("Alert Distance units", unit_str, 16,SSD_ALIGN_RIGHT|SSD_TEXT_NORMAL_FONT|SSD_END_ROW);
        ssd_widget_set_offset(text_w, 0, offset_y);
        ssd_widget_add(widget, text_w);
        ...
        if (group_flag != 0){
           ...
           ssd_widget_set_color(text,"#008888", NULL);
           ssd_widget_add(widget, text);
        }
        else{
           AlertStr[0] = 0;
