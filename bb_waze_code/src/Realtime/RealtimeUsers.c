...
#if (defined(__SYMBIAN32__) && !defined(TOUCH_SCREEN)) || defined (RIMAPI)
    ShowEditbox(roadmap_lang_get("Chit chat"), "", post_comment_keyboard_callback,
          g_user, EEditBoxEmptyForbidden | EEditBoxStandard );
#else
...
#if ((defined(__SYMBIAN32__) && !defined(TOUCH_SCREEN)) || defined(IPHONE_NATIVE) || defined (RIMAPI) || defined(ANDROID))
    ShowEditbox(roadmap_lang_get("Chit chat"), "", post_comment_keyboard_callback,
            user, EEditBoxEmptyForbidden | EEditBoxStandard );
#else
   ssd_show_keyboard_dialog(  roadmap_lang_get("Chit chat"),
                              NULL,
                              post_comment_keyboard_callback,
...
//	printf("now: %d ts_now: %4d-%2d-%2dT%2d:%2d:%2d\n",
//	   now,1900+ts_now.tm_year,ts_now.tm_mon,ts_now.tm_mday,ts_now.tm_hour,ts_now.tm_min,ts_now.tm_sec);
   ...
   ssd_widget_set_color(text,"#f6a201", NULL);
   ssd_widget_add(position_con, text);
   flags = SSD_POINTER_LOCATION|SSD_ROUNDED_BLACK;
#if 0//RIMAPI
   flags = flags |SSD_DIALOG_FLOAT_MAX_WIDTH;
#endif
   popup = ssd_popup_new("UsersPopUPDlg", "", NULL, SSD_MAX_SIZE, SSD_MIN_SIZE, &position,flags );
