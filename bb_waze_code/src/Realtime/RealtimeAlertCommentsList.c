...
#define ALERTS_COMMENTS_DIALOG_NAME "alert_comments"
...
int RealtimeAlertCommentsList(int iAlertId)
{
   ...
   int big_font_size = 16;
   int medium_font_size = 14;
   ...
   dialog = ssd_dialog_new (ALERTS_COMMENTS_DIALOG_NAME,
...
#ifdef RIMAPI
   ssd_contextmenu_menu_button_register(on_options_comments,ALERTS_COMMENTS_DIALOG_NAME);
#endif
   ...
   text = ssd_text_new ("Alert Disatnce", dist_str, big_font_size,SSD_ALIGN_CENTER|SSD_END_ROW);
   ssd_widget_add(image_con, text);

   text = ssd_text_new ("Alert Disatnce", unit_str, medium_font_size,SSD_ALIGN_CENTER);
   ...
   text = ssd_text_new("Distance_str", DistanceStr, big_font_size,SSD_END_ROW);
       ...
       if (ssd_widget_rtl(NULL)){
          if (CommentEntry->comment.bCommentByMe)
          {
            if ( roadmap_screen_is_hd_screen() )
             ...
             ssd_widget_set_offset(text_container, 14, 2);
       }
       else{
...
//       }
   ...
   text = ssd_text_new("Commnet txt",roadmap_lang_get("Add a comment"),big_font_size,SSD_ALIGN_CENTER|SSD_ALIGN_VCENTER);
   ssd_widget_set_color(text, "#206892",NULL);
