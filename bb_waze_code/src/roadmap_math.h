...
#ifdef J2ME
INLINE_DEC int roadmap_math_get_distance_from_segment
   ...
   return NOPH_RoadMapMath_distFromSeg(position, position1, position2, intersection, which);
}
#endif
...
INLINE_DEC int roadmap_math_line_is_visible (const RoadMapPosition *point1,
   ...
   if ((point1->longitude > RoadMapContext.visibility.east) &&
       (point2->longitude > RoadMapContext.visibility.east)) {
      return 0;
   }

   if ((point1->longitude < RoadMapContext.visibility.west) &&
       (point2->longitude < RoadMapContext.visibility.west)) {
      return 0;
   }

   if ((point1->latitude > RoadMapContext.visibility.north) &&
       (point2->latitude > RoadMapContext.visibility.north)) {
      return 0;
   }

   if ((point1->latitude < RoadMapContext.visibility.south) &&
       (point2->latitude < RoadMapContext.visibility.south)) {
      return 0;
   }
...
INLINE_DEC int roadmap_math_point_is_visible (const RoadMapPosition *point) {
   ...
   if ((point->longitude > RoadMapContext.visibility.east) ||
...
INLINE_DEC int roadmap_math_get_zoom_factor (void) {

   return RoadMapContext.zoom_factor;
}


INLINE_DEC int roadmap_math_is_visible (const RoadMapArea *area) {

   if (area->west > RoadMapContext.visibility.east ||
       area->east < RoadMapContext.visibility.west ||
       area->south > RoadMapContext.visibility.north ||
       area->north < RoadMapContext.visibility.south)
   {
       return 0;
   }

   if (area->west >= RoadMapContext.visibility.west &&
       area->east < RoadMapContext.visibility.east &&
       area->south > RoadMapContext.visibility.south &&
       area->north <= RoadMapContext.visibility.north)
   {
       return 1;
   }
 ...
 * -1 (partially visible).
 */
int roadmap_math_get_zoom_factor   (void);
