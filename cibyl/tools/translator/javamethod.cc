...
bool ExceptionHandler::pass2()
{
  ...
  emit->bc_invokestatic("int32 [WazeWP7]%sCRunTime::registerObject(object)",
  ...
  emit->bc_pushconst(0);
  emit->bc_pushconst(0);
  emit->bc_invokestatic("%s", dstMethod->getJavaMethodName(dstClass->getName()));
  emit->bc_pop();
...
char *JavaMethod::getJavaMethodName(const char *classname)
{
//  if ( this->javaName )
//    return this->javaName;
  ...
  int len = 1024;//strlen(this->functions[0]->getName()) + n + 8;
  ...
  if ( config->threadSafe )
    {
      if ( this->returnSize() == 2)
		...
		sprintf(type,"%s","int64");
      else if ( this->returnSize() == 1)
        //this->javaName[o++] = 'I';
		sprintf(type, "%s", "int32");
      else
        //this->javaName[o++] = 'V';
		sprintf(type, "%s", "void");
    }
  ...
  else
  {
	  ...
	  if (strcmp(getName(), "fcall") == 0)
	  {
		  theclassname = (char*)xcalloc(len+3+10, sizeof(char));
		  sprintf(theclassname, "[WazeWP7]%s::", classname);
	  }
	  else
	  {
		  theclassname = (char*)xcalloc(len+3, sizeof(char));
		  sprintf(theclassname, "%s::", classname);
	  }		  
  }

  sprintf(this->javaName, "%s %s%s(", type, theclassname, this->getName());
  for (int i=0; i<n; i++)
  {
	  this->javaName = strcat(this->javaName, "int32");
	  if (i!=(n-1))
		  this->javaName = strcat(this->javaName, ",");
  }

//  for (; n > 0; n--, o++)
  //  this->javaName[o] = 'int32';
  this->javaName = strcat(this->javaName, ")");
...
bool JavaMethod::pass2()
{
  ...
  emit->generic("\n.method public static %s\n{\n"
				".maxstack %d\n"
                "",//.limit locals %d\n",
                this->getJavaMethodName(NULL),
                this->getMaxStackHeight() + 2);//, regalloc->getNumberOfLocals());

  char registers_str[3000] = "";

  int mem_index = -1;
  for (int i=0; i< N_REGS; i++)
  {
	  if (this->registerUsage[i] && (strcmp(mips_reg_strings[i], "mem") == 0))
	  {
		  mem_index = i;
		  break;
	  }
  ...
  char *sorted_registers[N_REGS];
  for (int i = 0; i < N_REGS; i++)
  {
	  sorted_registers[i] = NULL;
  }
  ...
  for (int i = 0; i < N_REGS; i++)
  {
	  ...
	  {
		sorted_registers[ regalloc->regToLocal((MIPS_register_t)i) ] = (char*)mips_reg_strings[i];
	  }
  }
  ...
  for (int i = 0; i < N_REGS; i++)
  {
	  if (sorted_registers[i] != NULL)
	  {
		  if (i != 0)
		  {
			  strcat(registers_str, ",");
		  }
		  if (strcmp(sorted_registers[i], "mem") == 0)
		  {
			  strcat(registers_str, "int32[] mem");
		  }
		  else
		  {
			  strcat (registers_str, "int32 ");
			  strcat (registers_str, sorted_registers[i]);
		  }
	  }
  }

    //.locals init (int32 sp , int32 a0, int32 a1, int32 a2, int32 a3, int32 v0, int32 v1, int32[] mem, int32 ra)
    emit->generic(".locals init (%s)\n\n", registers_str);
  ...
  for (int i = 0; i < N_REGS; i++)
    {
	  ...
	  /* These are passed as arguments, map argument to local */
      if (this->registerIsArgument((MIPS_register_t)i))
	  {
	    if (regalloc->regToLocal((MIPS_register_t)i) >= 0)
		{
			emit->generic("\tldarg %d\n", regalloc->regToLocal((MIPS_register_t)i));
			emit->generic("\tstloc %d\n", regalloc->regToLocal((MIPS_register_t)i));
		}
		continue;
	  }
...
//            emit->bc_getstatic( "%sCRunTime/memory [I",
            emit->bc_getstatic( "int32[] [WazeWP7]%sCRunTime::memory",
                controller->getJasminPackagePath());
          else
            emit->bc_pushconst(0);
  ...
  if (config->threadSafe)
  ...
  else
    {
      ...
      if (this->returnSize() == 2)
        {
          emit->bc_pushregister(R_V1);
          emit->bc_putstatic("int32 [WazeWP7]%sCRunTime::saved_v1",
              controller->getJasminPackagePath());
        }
      if (this->returnSize() >= 1)
        {
...
int JavaMethod::returnSize()
{
  panic_if(this->m_returnSize < 0 || this->m_returnSize > 2,
      ...
      this->getJavaMethodName(NULL), this->m_returnSize);
