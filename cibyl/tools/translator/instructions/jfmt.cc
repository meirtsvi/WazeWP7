...
public:
	...
	virtual bool pass2()
	{
		...
		emit->bc_invokestatic("%s%s",
			controller->getJasminPackagePath(), this->dstMethod->getJavaMethodName(dstClass->getName()));
		...
		else
		{
			if (this->dstMethod->returnSize() == 2)
			{
				emit->bc_getstatic("int32 [WazeWP7]%sCRunTime::saved_v1",
...
{
	bool requires_zero_push;
public:
	OneRegisterConditionalJump(const char *what, uint32_t address, int opcode, MIPS_register_t rs,
		int32_t extra, bool requires_zero_push) : BranchInstruction(address, opcode, rs, R_ZERO, R_ZERO, extra)
	{
		this->bc = what;
		this->dst = (this->address + 4) + (this->extra << 2);
		this->requires_zero_push = requires_zero_push;
	}
