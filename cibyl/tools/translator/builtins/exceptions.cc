...
public:
  ...
  bool pass2()
  {
     ...
     * just rethrow it */
    emit->bc_invokevirtual("int32 %sSetjmpException::getCookie",
    ...
    /* Put value in V0 */
    emit->bc_invokevirtual("int32 %sSetjmpException::getValue",
        controller->getJasminPackagePath());
...
//    emit->bc_getstatic("%sCRunTime/objectRepository [Ljava/lang/Object;",
    emit->bc_getstatic("object[] [WazeWP7]%sCRunTime::objectRepository",
        controller->getJasminPackagePath());
    emit->bc_pushregister(R_A0);
    emit->bc_aaload();
//    emit->bc_checkcast("java/lang/Throwable");
    emit->bc_checkcast("Exception");
